...
if (process.env.DASHBOARD_DATA_BASELINE) {
  execFileSync("npm", ["run", "verify:dashboard-data-invariants", "--", process.env.DASHBOARD_DATA_BASELINE], { cwd: process.cwd(), env, stdio: "inherit" });
} else {
  throw new Error("DASHBOARD_DATA_BASELINE is required; capture it before deployment with npm run verify:dashboard-data-invariants.");
}
