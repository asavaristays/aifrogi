...
async function main() {
  ...
  const mailboxEmail = process.env.BOOKING_INBOX_EMAIL || "info@aifrogi.com";
  const mailboxPassword = process.env.BOOKING_INBOX_PASSWORD || "";
  if (!mailboxPassword) throw new Error("Booking mailbox is not configured.");
  const transporter = nodemailer.createTransport({ host: "smtp.hostinger.com", port: 465, secure: true, auth: { user: mailboxEmail, pass: mailboxPassword } });
  const result = await transporter.sendMail({
    from: `"AiFrogi" <${mailboxEmail}>`,
    to: recipient,
