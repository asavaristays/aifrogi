...
async function main() {
  ...
  const mailboxEmail = process.env.BOOKING_INBOX_EMAIL?.trim() || "info@aifrogi.com";
  const mailboxPassword = process.env.BOOKING_INBOX_PASSWORD || "";
  if (!mailboxPassword) throw new Error("AiFrogi mailbox is not configured.");
  const transporter = nodemailer.createTransport({
    ...
    socketTimeout: 20000,
    auth: { user: mailboxEmail, pass: mailboxPassword }
  });
  ...
  console.log(JSON.stringify({ status: "SMTP_ACCEPTED", recipient, messageId: result.messageId, accepted: result.accepted, rejected: result.rejected, executed: summary.executed, passed: summary.passed, failed: summary.failed }));
}
