import os,sys,json,subprocess,math
from PIL import Image,ImageDraw,ImageFont,ImageFilter
C=json.loads(sys.argv[1]);os.makedirs('/home/user/finalfilm',exist_ok=True);os.chdir('/home/user/finalfilm')
def run(a):subprocess.run(a,check=True,stdout=subprocess.DEVNULL)
def fetch(u,p):run(['curl','-f','-sS','-L','-A','Mozilla/5.0',u,'-o',p])
fetch(C['phone'],'phone.mp4');fetch(C['hotel'],'hotel.mp4')
fetch('https://d2ol7oe51mr4n9.cloudfront.net/user_3HgKfuLfI7SalGtcrGxZ6Tbs2Zu/dba8f38a-2d87-4726-9392-c1364f5223fa.mp4','old.mp4')
fetch('https://aifrogi.com/brand/aifrogi-logo-white.png','logo.png')
fetch('https://aifrogi.com/brand/aifrogi-sovereign-bot.png','bot.png')
W,H,F=720,1280,30
gold='#e5c879';white='#f6f5ef';grey='#b6b5b0'
fonts={}
def font(n):
 if n not in fonts:fonts[n]=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',n)
 return fonts[n]
def text(d,s,y,n=30,c=white,x=None):
 w=d.textbbox((0,0),s,font=font(n))[2]
 if x is None:x=(W-w)//2
 assert x>=35 and x+w<=685,(s,x,w)
 d.text((x,y),s,font=font(n),fill=c)
logo=Image.open('logo.png').convert('RGBA');logo=logo.crop(logo.getbbox())
mark=logo.copy();mark.thumbnail((152,65),Image.Resampling.LANCZOS)
bot=Image.open('bot.png').convert('RGB')
def ease(t):return 1-(1-max(0,min(1,t)))**3
def bg(t):
 im=Image.new('RGB',(W,H),(5,6,8));d=ImageDraw.Draw(im)
 # Quiet moving light traces, not a slide background.
 for j in range(8):
  x=int(360+270*math.sin(t*.22+j*.64));y=int(650+320*math.cos(t*.18+j*.64))
  d.ellipse((x-2,y-2,x+2,y+2),fill=(85,69,34))
 return im
def intro(t):
 im=bg(t).convert('RGBA')
 l=logo.copy();l.thumbnail((330,140),Image.Resampling.LANCZOS)
 im.alpha_composite(l,((W-l.width)//2,300))
 d=ImageDraw.Draw(im)
 text(d,'AI Business',540,48,gold);text(d,'Bot Family',602,48,gold)
 text(d,'+91-7410582898',830,34)
 text(d,'info@aifrogi.com',890,27,grey);text(d,'aifrogi.com',939,29,gold)
 return im.convert('RGB')
def mascot(t):
 s=1+.025*math.sin(t*.35);ww=int(720*s);hh=int(900*s)
 im=bg(t);b=bot.resize((ww,hh),Image.Resampling.LANCZOS);im.paste(b,((720-ww)//2,int(195+8*math.sin(t*.8))))
 return im
def chrome(im,caption='',note=''):
 layer=Image.new('RGBA',(W,H));d=ImageDraw.Draw(layer)
 for y in range(180):d.line((0,y,W,y),fill=(0,0,0,int(110*(1-y/180))))
 for y in range(990,H):d.line((0,y,W,y),fill=(0,0,0,int(205*(y-990)/290)))
 im=Image.alpha_composite(im.convert('RGBA'),layer)
 im.alpha_composite(mark,((720-mark.width)//2,78))
 d=ImageDraw.Draw(im)
 if caption:text(d,caption,1120,42)
 if note:text(d,note,1170,19,grey)
 return im.convert('RGB')
def bubble(d,box,lines,selected=False):
 x,y,w,h=box;d.rounded_rectangle((x,y,x+w,y+h),24,fill='#14171d',outline=gold if selected else '#44433e',width=2)
 for j,s in enumerate(lines):text(d,s,y+22+j*39,25,gold if j==0 else white,x+24)
def report(t):
 im=bg(t);d=ImageDraw.Draw(im)
 # Minimal crop-style reconstruction of the inspected Reports operating picture.
 d.rounded_rectangle((58,310,662,940),30,fill='#182720',outline='#40574a',width=2)
 text(d,'Reports',350,38,white)
 for j,(label,value,color) in enumerate([('Inbound',42,'#51ce97'),('Outbound',44,'#81b7f9'),('Answered contacts',13,'#e8b859')]):
  y=470+j*145
  text(d,label,y,28,grey,92);text(d,str(value),y-8,44,white,560)
  d.rounded_rectangle((92,y+66,625,y+75),4,fill='#34453c')
  d.rounded_rectangle((92,y+66,92+int(533*value/44),y+75),4,fill=color)
 text(d,'6 September 2026',980,20,grey)
 return chrome(im,'Clear insights.')
def handover(t):
 im=bg(t);d=ImageDraw.Draw(im)
 d.rounded_rectangle((58,300,662,980),30,fill='#f8f6f1',outline='#d6ccba',width=2)
 text(d,'Team Inbox',345,38,'#171714')
 d.rounded_rectangle((90,425,630,540),18,fill='#fff4df')
 text(d,'Website visitor',446,29,'#171714',116)
 text(d,'Human needed',490,23,'#977219',116)
 if t<1.7:
  d.rounded_rectangle((92,605,628,735),16,fill='#ffffff')
  text(d,'Can I speak to your team?',650,28,'#171714')
 else:
  d.rounded_rectangle((92,595,530,710),16,fill='#ffffff')
  text(d,'Can I speak to your team?',635,25,'#171714',110)
  d.rounded_rectangle((165,745,628,845),16,fill='#eae3d3')
  text(d,'Conversation included',778,26,'#171714',190)
 d.rounded_rectangle((90,895,630,952),12,outline='#cfcbc2',width=2)
 text(d,'Write a reply…',908,23,'#8b877c',114)
 return chrome(im,'Human help.')

def english_hotel(im,t):
 im=im.copy();d=ImageDraw.Draw(im)
 # Controlled phone screen covers all generated writing, not the full video.
 d.rounded_rectangle((169,219,591,1081),50,fill='#0a0c10',outline='#b49b60',width=3)
 d.rounded_rectangle((187,237,573,1063),38,fill='#141920')
 d.ellipse((349,246,371,268),fill='#030405')
 text(d,'HotelGPT',290,29,gold)
 stage=min(3,int(t/3.25))
 title=['Choose dates','Choose your room','Review your stay','Room confirmed'][stage]
 text(d,title,352,26)
 if stage==0:
  text(d,'SEPTEMBER',420,22,grey)
  for n in range(1,29):
   col=(n-1)%7;row=(n-1)//7;x=212+col*43;y=495+row*65
   selected=n in [12,13,14] and t>1
   if selected:d.rounded_rectangle((x-3,y-7,x+32,y+35),8,fill='#a9893d')
   d.text((x,y),str(n),font=font(21),fill=white)
  text(d,'12–14 Sep  ·  2 guests',830,22,gold)
 elif stage==1:
  im.paste(room,(205,423));d=ImageDraw.Draw(im)
  text(d,'Garden Suite',645,26,gold)
  text(d,'Two guests · Two nights',691,20,grey)
  d.rounded_rectangle((213,772,548,850),15,outline=gold,width=2)
  text(d,'Select room',797,24)
 elif stage==2:
  for y,s in [(460,'Garden Suite'),(530,'12–14 September'),(600,'2 guests · 2 nights')]:text(d,s,y,23,gold if y==460 else white)
  d.rounded_rectangle((214,766,546,846),16,fill='#9b7c32');text(d,'Confirm room',791,24)
 else:
  k=ease((t-9.75)/.8);d.ellipse((278,453,442,617),outline='#53c999',width=4)
  if k>.3:d.line((310,531,343,562,412,493),fill='#53c999',width=7)
  text(d,'Booking complete',675,25,gold)
  text(d,'Fictional booking only',729,20,grey)
 text(d,'Availability · No payment',970,18,grey)
 return im

def family(t,base):
 im=Image.blend(base.convert('RGB').filter(ImageFilter.GaussianBlur(40)),Image.new('RGB',(W,H),(4,5,7)),.60)
 d=ImageDraw.Draw(im);idx=min(2,int(t/2));labels=[('ClinicGPT','Appointment enquiries'),('PropertyGPT','Property discovery'),('FlowCart','Shopping assistance')]
 label,sub=labels[idx];u=(t%2)/2
 shift=int(60*(1-ease(u*4)))
 # Typography flies through the same cinematic environment; no card grid.
 text(d,label,530+shift,48,gold);text(d,sub,607+shift,28)
 return chrome(im,'One bot family.')
def ending(t):
 im=bg(t);im.paste(bot.resize((420,525),Image.Resampling.LANCZOS),(150,240));shade=Image.new('RGBA',(W,H),(0,0,0,0));sd=ImageDraw.Draw(shade)
 sd.rectangle((0,780,720,1280),fill=(5,6,8,255));im=Image.alpha_composite(im.convert('RGBA'),shade)
 l=logo.copy();l.thumbnail((270,110));im.alpha_composite(l,((720-l.width)//2,140))
 d=ImageDraw.Draw(im);text(d,'Less waiting. More business.',815,32)
 text(d,'Start your 15-day trial',880,26,gold)
 text(d,'+91-7410582898',975,34)
 text(d,'info@aifrogi.com',1035,25,grey);text(d,'aifrogi.com',1080,25,gold)
 return im.convert('RGB')
run(['ffmpeg','-y','-v','error','-ss','9','-i','hotel.mp4','-frames:v','1','room-source.png'])
room=Image.open('room-source.png').convert('RGB').crop((145,585,720,1030)).resize((310,195),Image.Resampling.LANCZOS)
out=subprocess.Popen(['ffmpeg','-y','-v','error','-f','rawvideo','-pix_fmt','rgb24','-s','720x1280','-r','30','-i','-','-an','-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p','picture.mp4'],stdin=subprocess.PIPE)
samples=[];globalframe=0
def write(im):
 global globalframe
 if globalframe%150==0:samples.append(im.resize((240,426)))
 out.stdin.write(im.convert('RGB').tobytes());globalframe+=1
def footage(path,seconds,start=0,rate=1):
 f=f"scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,setpts=PTS/{rate},fps=30"
 pr=subprocess.Popen(['ffmpeg','-v','error','-ss',str(start),'-i',path,'-vf',f,'-t',str(seconds),'-f','rawvideo','-pix_fmt','rgb24','-'],stdout=subprocess.PIPE)
 for i in range(seconds*30):
  raw=pr.stdout.read(W*H*3)
  if len(raw)!=W*H*3:raise RuntimeError('Missing video frame')
  yield i/30,Image.frombytes('RGB',(W,H),raw)
 pr.stdout.close();pr.wait()
for i in range(90):write(intro(i/30))
for t,im in footage('phone.mp4',4,start=1):write(chrome(im,'Every enquiry matters.'))
for i in range(120):write(chrome(mascot(i/30),'Your AI assistant.'))
for t,im in footage('hotel.mp4',12,rate=15/12):
 stage=min(3,int(t/3));caps=['Choose dates.','Choose your room.','Review your stay.','Room confirmed.']
 write(chrome(english_hotel(im,t*13/12),caps[stage]))
for t,im in footage('hotel.mp4',6,start=9):write(family(t,english_hotel(im,9+t)))
for i in range(150):write(report(i/30))
for i in range(150):write(handover(i/30))
for i in range(180):write(ending(i/30))
out.stdin.close();assert out.wait()==0
assert globalframe==1350
run(['ffmpeg','-y','-v','error','-i','picture.mp4','-i','old.mp4','-map','0:v','-map','1:a','-c:v','copy','-c:a','copy','-t','45','-movflags','+faststart','final.mp4'])
sheet=Image.new('RGB',(720,1278))
for j,s in enumerate(samples):sheet.paste(s,((j%3)*240,(j//3)*426))
sheet.save('review.jpg',quality=92)
print(subprocess.check_output(['ffprobe','-v','error','-show_entries','format=duration,size','-of','json','final.mp4']).decode(),flush=True)
for name,u in zip(['final.mp4','review.jpg'],C['uploads']):
 res=subprocess.check_output(['curl','-f','-sS','-w','%{http_code}','-X','PUT','-H','Content-Type: '+u['content_type'],'--upload-file',name,u['upload_url']]).decode()
 assert res.endswith('200'),res
 print('UPLOADED '+u['media_id']+' HTTP '+res,flush=True)
