...
export function calculateProductFlow(input: ProductFlowInput): ProductFlow {
  ...
  const candidates = [
    ...
    stage({ id: "automation", label: "Automation proof", description: "At least one workflow completes with retries and human control available.", owner: "AiFrogi", complete: input.completedAutomationCount > 0 && input.deadAutomationCount === 0, blocked: input.deadAutomationCount > 0, evidence: input.deadAutomationCount > 0 ? `${input.deadAutomationCount} automation job${input.deadAutomationCount === 1 ? "" : "s"} need intervention` : input.completedAutomationCount > 0 ? `${input.completedAutomationCount} completed automation job${input.completedAutomationCount === 1 ? "" : "s"}` : "No workflow execution has been proven", action: input.deadAutomationCount > 0 ? "Resolve automation failure" : "Run a workflow test", href: "/workflows" }),
