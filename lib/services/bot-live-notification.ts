...
export async function notifyBotLive(organizationId: string, actorEmail: string, options: { force?: boolean } = {}) {
  const db = getDb();
  ...
  if (lastSent && !options.force) return { accepted: true, message: "Live confirmation was already accepted by the mail server." };
  let accepted = false;
  try {
