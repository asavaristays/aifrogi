...
export function CustomerReviewActions({ organizationId, kycStatus, organizationStatus }: { organizationId: string; kycStatus: string; organizationStatus: string }) {
  ...
  return (
    <section className="rounded-lg border border-black/6 bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm text-[#6d7487]">Approve verified details or explain exactly what the customer must update.</p>
      <textarea className="mt-4 min-h-24 w-full rounded-md border border-black/10 bg-[#f8faf9] px-3 py-3 text-sm outline-none focus:border-[#c725ba]" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason required when rejecting" />
