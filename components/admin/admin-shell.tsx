...
export function AdminShell({ userEmail, children }: { userEmail: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#efede7] text-[#101010] lg:grid lg:grid-cols-[280px_minmax(0,1fr)]">
    ...
    <aside id="admin-navigation" className={`${open ? "fixed inset-x-0 top-[72px] z-30 block" : "hidden"} max-h-[calc(100vh-72px)] overflow-y-auto bg-[#070707] p-4 text-white shadow-2xl lg:sticky lg:top-0 lg:block lg:h-screen lg:max-h-none lg:p-5`}>
      <div className="flex h-full flex-col rounded-[30px] border border-white/8 bg-[radial-gradient(circle_at_top,rgba(178,135,40,.17),transparent_34%),#101010] p-4 shadow-[0_30px_80px_rgba(0,0,0,.28)]">
        <Link href="/admin" onClick={() => setOpen(false)} className="hidden px-3 pb-7 pt-4 lg:block"><Image src="/brand/aifrogi-logo-white.png" alt="AiFrogi" width={800} height={300} priority className="h-auto w-[168px] grayscale contrast-125" /><span className="mt-3 block text-[10px] font-semibold uppercase tracking-[.22em] text-[#e2c66d]">Sovereign operations</span></Link>

        <Link href="/admin/onboard" onClick={() => setOpen(false)} className="group flex min-h-14 items-center justify-between rounded-2xl bg-[#b28728] px-4 font-bold text-white shadow-[0_18px_40px_rgba(178,135,40,.24)] transition hover:-translate-y-0.5 hover:bg-[#d4af37] hover:text-[#101010]"><span className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-full bg-black/14"><Icon name="sparkles" /></span>Onboard client</span><Icon name="arrow-right" /></Link>

        <nav aria-label="Super Admin navigation" className="mt-6 space-y-1.5">
          {navigation.map((item) => {
            const active = item.href === "/admin" ? pathname === item.href : pathname.startsWith(item.href);
            return <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className={`flex min-h-12 items-center gap-3 rounded-2xl px-3.5 text-sm font-semibold transition ${active ? "bg-white text-[#101010] shadow-[0_12px_28px_rgba(0,0,0,.22)]" : "text-white/62 hover:bg-white/7 hover:text-white"}`}><span className={`grid h-8 w-8 place-items-center rounded-xl ${active ? "bg-[#f3e5b5] text-[#6d5310]" : "bg-white/6 text-[#e2c66d]"}`}><Icon name={item.icon} /></span>{item.label}</Link>;
          })}
        </nav>
