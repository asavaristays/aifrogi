  ...
  return (
    <section className="rounded-lg border border-black/6 bg-white p-6 shadow-sm lg:col-span-2">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#c725ba]">SaaS configuration</p>
          <h2 className="mt-2 text-lg font-black">WhatsApp AI bot</h2>
          <p className="mt-1 text-sm text-[#6d7487]">Replies are English-only and use this client&apos;s enabled services.</p>
        </div>
        <label className="flex items-center gap-3 text-sm font-black">
          <input
            ...
            className="size-5 accent-[#c725ba]"
          />
          Automation active
        </label>
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div>
          <label className="block text-xs font-black uppercase tracking-[0.12em] text-[#6d7487]" htmlFor="client-plan">Client plan</label>
          <select id="client-plan" value={plan} onChange={(event) => setPlan(event.target.value)} className="mt-2 h-11 w-full rounded-md border border-black/10 bg-[#f8faf9] px-3 text-sm font-bold outline-none focus:border-[#c725ba]">
            {PLAN_OPTIONS.map((option) => <option key={option} value={option}>{option.replaceAll("_", " ")}</option>)}
          </select>

          <p className="mt-5 text-xs font-black uppercase tracking-[0.12em] text-[#6d7487]">Enabled capabilities</p>
          <div className="mt-3 space-y-2">
            {WHATSAPP_BOT_SERVICE_OPTIONS.map((option) => (
              <label key={option.key} className="flex items-center gap-3 rounded-md border border-black/6 px-3 py-3 text-sm font-semibold">
                <input type="checkbox" checked={configuration.serviceBuckets.includes(option.key)} onChange={() => toggleService(option.key)} className="size-4 accent-[#c725ba]" />
                {option.label}
              </label>
            ))}
          </div>
        ...
        <div>
          <label className="block text-xs font-black uppercase tracking-[0.12em] text-[#6d7487]" htmlFor="welcome-message">Opening message</label>
          <textarea
            id="welcome-message"
          ...
          <p className="mt-5 text-xs font-black uppercase tracking-[0.12em] text-[#6d7487]">First-reply preview</p>
      ...
      {message ? <p className={`mt-4 text-sm font-semibold ${message.includes("saved") ? "text-[#c725ba]" : "text-[#a3342b]"}`}>{message}</p> : null}
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) {
  return (
    <label className="flex items-center gap-3 rounded-md border border-black/6 px-3 py-3 text-sm font-semibold">
      <input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-4 accent-[#c725ba]" />
      {label}
    </label>
  );
}
