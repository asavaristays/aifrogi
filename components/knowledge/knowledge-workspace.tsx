...
type TenantProfileDraft = { status: "DRAFT" | "NEEDS_REVIEW"; businessName: string; about: string; services: string[]; prices: string[]; hours: string[]; phones: string[]; emails: string[]; addresses: string[]; policies: string[]; team: string[]; socialLinks: string[]; bookingLinks?: string[]; mapLinks?: string[]; amenities?: string[]; rooms?: string[]; availability?: string[]; access?: string[]; missing: string[]; conflicts: Array<{ key: string; values: string[] }> };
type TenantTruthReview = { counts: { conflicts: number; stale: number; commercial: number }; items: Array<{ id: string; reason: string; priority: string; field: string; values: string[]; sourceUrls: string[]; action: string }> };
type TenantEntitySummary = { entityType: "PROPERTY" | "PRODUCT" | "SERVICE"; entityId: string; name: string; aliases?: string[]; vocabulary?: string[]; sourceUrl: string; imageUrls: string[]; facts: Array<{ field: string; value: string }> };
type TenantLearningTopic = { id: string; title: string; suggestedQuestion: string; occurrenceCount: number; questionCount: number; gapIds: string[]; exampleQuestions: string[]; lastAskedAt: string; status: "DRAFT_REVIEW" };
type TenantChangeSet = { status: "INITIAL" | "UNCHANGED" | "CHANGED"; detectedAt: string; addedPages: string[]; removedPages: string[]; changedPages: string[]; addedFacts: string[]; removedFacts: string[]; requiresReview: boolean };
type TenantFreshness = { status: "CURRENT" | "DUE"; nextRefreshAt: string | null; ageHours: number | null };
type Summary = { settings: KnowledgeSettings; pages: KnowledgePageSummary[]; tenantProfileDraft?: TenantProfileDraft | null; tenantEntities?: TenantEntitySummary[]; truthReview?: TenantTruthReview | null; changeSet?: TenantChangeSet | null; freshness?: TenantFreshness; learningTopics?: TenantLearningTopic[]; propertyId: string | null; documents: KnowledgeDocumentSummary[]; entries: KnowledgeEntrySummary[]; gaps: KnowledgeGapSummary[]; previews?: KnowledgePreviewSummary[]; flags?: KnowledgeFlagSummary[]; improvementRoutes?: ImprovementRouteSummary[]; verification?: VerificationSummary | null; kbGateEnabled?: boolean; isTrial?: boolean; testComplete?: boolean; botStatus?: string };
...
export function KnowledgeWorkspace({
  ...
  async function syncWebsite() {
    ...
    try {
      ...
      setSummary((current) => ({ ...current, settings: payload.settings, pages: payload.pages, tenantProfileDraft: payload.tenantProfileDraft, tenantEntities: payload.tenantEntities, truthReview: payload.truthReview, changeSet: payload.changeSet, freshness: payload.freshness }));
  ...
  async function connectWebsite() {
    ...
    try {
      ...
      setSummary((current) => ({ ...current, settings: syncedPayload.settings, pages: syncedPayload.pages, tenantProfileDraft: syncedPayload.tenantProfileDraft, tenantEntities: syncedPayload.tenantEntities, truthReview: syncedPayload.truthReview, changeSet: syncedPayload.changeSet, freshness: syncedPayload.freshness }));
  ...
  async function refreshGovernance() {
    ...
    setSummary((current) => ({ ...current, propertyId: payload.propertyId, documents: payload.documents || [], entries: payload.entries || [], gaps: payload.gaps || [], learningTopics: payload.learningTopics || [], changeSet: payload.changeSet, freshness: payload.freshness, previews: payload.previews || [], flags: payload.flags || [], improvementRoutes: payload.improvementRoutes || [], verification: payload.verification, kbGateEnabled: payload.kbGateEnabled }));
  }
  ...
  async function saveEntry(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/knowledge/entries", { method: editingEntryId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editingEntryId ? { id: editingEntryId, action: "EDIT_AND_APPROVE", question: entryQuestion, answer: entryAnswer, category: entryCategory } : { question: entryQuestion, answer: entryAnswer, category: entryCategory, gapId: entryGapId }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Could not save this answer.");
      await refreshGovernance();
      setEntryQuestion(""); setEntryAnswer(""); setEntryCategory("General"); setEntryGapId(undefined); setEditingEntryId(null);
      setNotice(editingEntryId ? "Correction saved, approved and made live." : payload.entry.status === "CONFLICT" ? "Answer saved with a possible conflict. Review it before approval." : "Answer saved as a draft. Approve it when ready.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "Could not save this answer."); }
    finally { setSavingEntry(false); }
  }

  async function reviewEntry(id: string, action: "FIELD_APPROVE" | "GENERATE_PREVIEW" | "PREVIEW_APPROVE" | "PREVIEW_REJECT" | "PAUSE" | "RECONFIRM" | "DELETE" | "RETIRE", previewId?: string, hasConflict = false) {
    ...
    const messages: Record<string, string> = {
      ...
      RETIRE: "Answer removed from bot use. Its audit history has been retained."
    };
    setNotice(messages[action] || "Answer updated.");
  }

  function startEditEntry(item: KnowledgeEntrySummary) {
    setEditingEntryId(item.id); setEntryQuestion(item.question); setEntryAnswer(item.answer); setEntryCategory(item.category); setEntryGapId(undefined);
  ...
  function cancelEditEntry() {
    setEditingEntryId(null); setEntryQuestion(""); setEntryAnswer(""); setEntryCategory("General"); setEntryGapId(undefined);
  }
  ...
  const manageableEntries = summary.entries.filter((item) => item.status !== "SUPERSEDED");
  ...
  const informationAdded = summary.pages.length > 0 || summary.documents.length > 0 || publishedCount > 0;
  const submitted = ["REVIEW_PENDING", "LIVE", "PAUSED"].includes(summary.botStatus || "");
  const remainingEssentials = Math.max(0, 5 - (summary.verification?.essentials?.covered || 0));
  const nextAction = !informationAdded
    ? { title: "Add your business information", copy: "Connect the website, upload a file, or add one verified answer.", href: "#start-information", label: "Add information" }
    : !ready
    ? { title: quickTrial ? `Approve ${remainingEssentials} required topic${remainingEssentials === 1 ? "" : "s"}` : "Approve the remaining required topics", copy: "Check the suggested answers and approve only information customers may receive.", href: "#manual-answer-form", label: "Review required answers" }
    : !summary.testComplete
    ? { title: "Test one customer question", copy: "Confirm that the bot answers correctly from your approved information.", href: "#test-your-bot", label: "Test my bot" }
    : !submitted
    ? { title: "Submit the bot for Super Admin review", copy: "Intelligence and testing are complete. Finish setup and submit; this does not make the bot public.", href: "/setup", label: "Continue to final approval" }
    : { title: summary.botStatus === "LIVE" ? "Your AI Bot is live" : "Submitted—awaiting Super Admin", copy: summary.botStatus === "LIVE" ? "Customers can now use the approved bot." : "No further client action is required. AiFrogi will email you after review.", href: "/setup", label: "View activation status" };
  ...
  return <div className="product-surface min-h-screen">
    ...
    <main className="mx-auto max-w-[1500px] space-y-5 px-5 py-6 sm:px-8">
      <section id="start-information" className="scroll-mt-5 overflow-hidden rounded-[28px] bg-[#080808] text-white shadow-[0_28px_80px_rgba(0,0,0,.18)]">
        ...
        <div className="grid border-t border-white/10 sm:grid-cols-2 xl:grid-cols-4">{[
          [informationAdded, "1", "Add information", "Website, document or manual answer"],
          ...
          [Boolean(summary.testComplete), "3", "Test an answer", summary.testComplete ? "Customer-question test recorded" : "Test one normal customer question"],
          [submitted, "4", "Submit for approval", summary.botStatus === "LIVE" ? "Approved and live" : submitted ? "Awaiting Super Admin" : "Final action in Setup"]
      ...
      <section className="rounded-2xl border-2 border-[#c89a24] bg-[#fffaf0] p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="product-eyebrow">Do this next</p><h2 className="mt-1 text-xl font-semibold">{nextAction.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">{nextAction.copy}</p></div><a href={nextAction.href} className="inline-flex min-h-12 shrink-0 items-center justify-center rounded-full bg-[#101010] px-6 text-sm font-bold text-white">{nextAction.label} →</a></div></section>
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="product-eyebrow">Learning lifecycle</p><h2 className="mt-1 text-xl font-semibold">Keep knowledge current and turn missed questions into reviewed answers.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">Website changes and repeated unanswered questions are prepared for review. AiFrogi never publishes learned content automatically.</p></div><span className={`status-pill ${summary.freshness?.status === "DUE" ? "status-warning" : "status-success"}`}>{summary.freshness?.status === "DUE" ? "Refresh due" : "Knowledge current"}</span></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-3"><Fact label="Next website check" value={summary.freshness?.nextRefreshAt ? formatDateTime(summary.freshness.nextRefreshAt) : "Sync required"} /><Fact label="Latest crawl change" value={summary.changeSet?.status === "CHANGED" ? `${summary.changeSet.changedPages.length + summary.changeSet.addedPages.length + summary.changeSet.removedPages.length} page changes need review` : summary.changeSet?.status === "UNCHANGED" ? "No source changes detected" : "Initial source captured"} /><Fact label="Draft learning topics" value={`${summary.learningTopics?.length || 0} awaiting review`} /></div>
        {summary.changeSet?.requiresReview ? <details className="mt-4 rounded-xl border border-[#eadcae] bg-[#fffaf0] p-4"><summary className="cursor-pointer text-sm font-semibold">Review detected website changes</summary><div className="mt-3 grid gap-2 sm:grid-cols-2"><Fact label="Pages added" value={String(summary.changeSet.addedPages.length)} /><Fact label="Pages changed" value={String(summary.changeSet.changedPages.length)} /><Fact label="Pages removed" value={String(summary.changeSet.removedPages.length)} /><Fact label="Fact changes" value={String(summary.changeSet.addedFacts.length + summary.changeSet.removedFacts.length)} /></div><p className="mt-3 text-xs text-[var(--text-muted)]">Changed website content remains source evidence only; commercial facts and customer answers still follow tenant approval.</p></details> : null}
        {summary.learningTopics?.length ? <div className="mt-4 grid gap-3 md:grid-cols-2">{summary.learningTopics.slice(0, 6).map((topic) => <article key={topic.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-4"><div className="flex items-start justify-between gap-3"><div><small className="font-bold uppercase tracking-[.12em] text-[var(--primary-strong)]">Suggested topic</small><h3 className="mt-1 font-semibold">{topic.title}</h3></div><span className="rounded-full bg-white px-2.5 py-1 text-xs text-[var(--text-muted)]">{topic.occurrenceCount} asks</span></div><p className="mt-3 text-sm">{topic.suggestedQuestion}</p>{canManage ? <button onClick={() => { setEntryQuestion(topic.suggestedQuestion); setEntryAnswer(""); setEntryCategory(topic.title.replaceAll(" · ", " ")); setEntryGapId(topic.gapIds[0]); document.getElementById("manual-answer-form")?.scrollIntoView({ behavior: "smooth" }); }} className="mt-3 text-xs font-semibold text-[var(--primary-strong)]">Review and draft answer →</button> : null}</article>)}</div> : <p className="mt-4 rounded-xl bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text-muted)]">No repeated unanswered customer topics are waiting for review.</p>}
      </section>

      {summary.tenantProfileDraft ? <section className="rounded-2xl border border-[var(--border)] bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="product-eyebrow">Website fact review</p><h2 className="mt-1 text-xl font-semibold">Confirm what we understood about your business.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">These are structured suggestions from your public website. They remain drafts until you approve answers below.</p></div><span className={`status-pill ${summary.tenantProfileDraft.conflicts.length ? "status-error" : "status-warning"}`}>{summary.tenantProfileDraft.conflicts.length ? `${summary.tenantProfileDraft.conflicts.length} conflict${summary.tenantProfileDraft.conflicts.length === 1 ? "" : "s"} to review` : "Draft · approval required"}</span></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Fact label="Business" value={summary.tenantProfileDraft.businessName} /><Fact label="Public contact" value={[...summary.tenantProfileDraft.phones, ...summary.tenantProfileDraft.emails, ...summary.tenantProfileDraft.addresses].join(" · ")} /><Fact label="Booking routes" value={summary.tenantProfileDraft.bookingLinks?.length ? `${summary.tenantProfileDraft.bookingLinks.length} found` : "Not found"} /><Fact label="Pricing found" value={summary.tenantProfileDraft.prices.length ? `${summary.tenantProfileDraft.prices.length} structured suggestion${summary.tenantProfileDraft.prices.length === 1 ? "" : "s"}` : "Not found"} /><Fact label="Services" value={summary.tenantProfileDraft.services.length ? `${summary.tenantProfileDraft.services.length} found` : "Not found"} /><Fact label="Rooms / products" value={summary.tenantProfileDraft.rooms?.length ? `${summary.tenantProfileDraft.rooms.length} found` : "Not found"} /><Fact label="Amenities" value={summary.tenantProfileDraft.amenities?.length ? `${summary.tenantProfileDraft.amenities.length} found` : "Not found"} /><Fact label="Location & access" value={summary.tenantProfileDraft.access?.length ? `${summary.tenantProfileDraft.access.length} found` : "Not found"} /></div>
        {summary.tenantProfileDraft.missing.length ? <div className="mt-4 rounded-xl bg-[#fff8e6] px-4 py-3 text-sm"><strong>Information to add:</strong> {summary.tenantProfileDraft.missing.join(", ").replaceAll("_", " ")}.</div> : null}
        {summary.truthReview?.items.length ? <details className="mt-4 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-4"><summary className="cursor-pointer text-sm font-semibold">Truth review queue · {summary.truthReview.items.length} item{summary.truthReview.items.length === 1 ? "" : "s"}</summary><div className="mt-3 grid gap-2 sm:grid-cols-3"><Fact label="Conflicts" value={String(summary.truthReview.counts.conflicts)} /><Fact label="Stale facts" value={String(summary.truthReview.counts.stale)} /><Fact label="Commercial confirmations" value={String(summary.truthReview.counts.commercial)} /></div><div className="mt-3 space-y-2">{summary.truthReview.items.slice(0, 12).map((item) => <article key={item.id} className="rounded-lg border border-[var(--border)] bg-white p-3"><div className="flex items-center justify-between gap-3"><strong className="text-xs uppercase tracking-wide">{item.field.replaceAll("_", " ")}</strong><span className={`status-pill ${item.priority === "CRITICAL" ? "status-error" : "status-warning"}`}>{item.reason.toLowerCase().replaceAll("_", " ")}</span></div><p className="mt-2 text-sm">{item.values.join(" · ")}</p><p className="mt-2 text-xs text-[var(--text-muted)]">{item.action}</p>{item.sourceUrls[0] ? <a className="mt-2 inline-block text-xs font-semibold text-[var(--primary-strong)]" href={item.sourceUrls[0]} target="_blank" rel="noreferrer">Inspect source ↗</a> : null}</article>)}</div></details> : null}
      </section> : null}
      {summary.tenantEntities?.length ? <section className="rounded-2xl border border-[var(--border)] bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="product-eyebrow">Entity catalogue</p><h2 className="mt-1 text-xl font-semibold">Businesses, properties, products and services the bot recognizes.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">Aliases and category vocabulary help the bot understand natural wording and small spelling mistakes. Every entity remains tied to its first-party source.</p></div><span className="status-pill status-success">{summary.tenantEntities.length} recognized</span></div>
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{summary.tenantEntities.slice(0, 18).map((entity) => <article key={`${entity.entityType}-${entity.entityId}`} className="rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-4"><div className="flex items-start justify-between gap-3"><div><small className="font-bold uppercase tracking-[.12em] text-[var(--primary-strong)]">{entity.entityType.toLowerCase()}</small><h3 className="mt-1 font-semibold">{entity.name}</h3></div><span className="rounded-full bg-white px-2.5 py-1 text-xs text-[var(--text-muted)]">{entity.facts.length} facts</span></div>{entity.aliases?.length ? <p className="mt-3 text-xs leading-5 text-[var(--text-muted)]"><strong className="text-[var(--text)]">Understands:</strong> {entity.aliases.slice(0, 4).join(" · ")}</p> : null}<a className="mt-3 inline-block text-xs font-semibold text-[var(--primary-strong)]" href={entity.sourceUrl} target="_blank" rel="noreferrer">Inspect source ↗</a></article>)}</div>
      </section> : null}
      ...
      <section className="grid items-start gap-5 xl:grid-cols-2">
        ...
        <div id="manual-answer-form" className="soft-card rounded-lg p-5"><p className="product-eyebrow">Step 2</p><h2 className="mt-1 text-lg font-semibold">Review and approve answers</h2><p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">Check each answer below. AiFrogi will guide you through one clear action at a time before anything can go live.</p>{canManage ? <details className="group mt-5 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)]"><summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-semibold"><span>{editingEntryId ? "Edit answer" : "+ Add an answer manually"}</span><span className="text-[var(--primary-strong)] group-open:rotate-90">→</span></summary><form onSubmit={saveEntry} className="space-y-3 border-t border-[var(--border)] p-4">{editingEntryId ? <p className="rounded-md bg-[var(--info-soft)] px-3 py-2 text-xs text-[#385d8e]">Review the complete answer below. Save and approve applies the correction immediately after AiFrogi&apos;s validation checks pass.</p> : null}<label className="block"><span className="field-label">Question a customer may ask</span><input className="product-input mt-2" value={entryQuestion} onChange={(event) => setEntryQuestion(event.target.value)} required placeholder="Example: What is your phone number?" /></label><label className="block"><span className="field-label">Correct answer</span><textarea className="product-input mt-2 min-h-28 resize-y" value={entryAnswer} onChange={(event) => setEntryAnswer(event.target.value)} required placeholder="Enter the exact answer the bot may use." /></label><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><label className="flex-1"><span className="field-label">Topic</span><input className="product-input mt-2" value={entryCategory} onChange={(event) => setEntryCategory(event.target.value)} /></label><div className="flex gap-2"><button disabled={savingEntry} className="min-h-11 rounded-md bg-[var(--primary-strong)] px-4 text-xs font-semibold text-white disabled:opacity-55">{savingEntry ? "Validating..." : editingEntryId ? "Save and approve" : "Save for review"}</button>{editingEntryId ? <button type="button" onClick={cancelEditEntry} className="min-h-11 rounded-md border border-[var(--border)] px-4 text-xs font-semibold">Cancel</button> : null}</div></div>{entryGapId ? <p className="rounded-md bg-[var(--info-soft)] px-3 py-2 text-xs text-[#385d8e]">This answer will resolve the selected unanswered question.</p> : null}</form></details> : null}<div className="mt-5 space-y-4">{manageableEntries.length ? manageableEntries.map((item, index) => <EntryRow key={item.id} item={item} position={index + 1} total={manageableEntries.length} preview={(summary.previews || []).find((preview) => preview.entryId === item.id && preview.status === "PENDING")} canManage={canManage} onReview={reviewEntry} onEdit={startEditEntry} />) : <EmptyState title="No answers to review" copy="Upload the completed Excel template, a supporting document, or add one answer manually." />}</div></div>
      </section>
...
function Fact({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-[var(--surface-soft)] px-4 py-3"><small className="field-label">{label}</small><p className="mt-1 break-words text-sm font-medium">{value || "Not found"}</p></div>;
}
...
function EntryRow({ item, position, total, preview, canManage, onReview, onEdit }: { item: KnowledgeEntrySummary; position: number; total: number; preview?: KnowledgePreviewSummary; canManage: boolean; onReview: (id: string, action: "FIELD_APPROVE" | "GENERATE_PREVIEW" | "PREVIEW_APPROVE" | "PREVIEW_REJECT" | "PAUSE" | "RECONFIRM" | "DELETE" | "RETIRE", previewId?: string, conflict?: boolean) => void; onEdit: (item: KnowledgeEntrySummary) => void }) {
  ...
  return <article className={`rounded-xl border p-4 ${published ? "border-emerald-200 bg-emerald-50/40" : "border-[var(--border)] bg-white"}`}>
    ...
    {canManage ? <div className="mt-4 flex flex-wrap items-center gap-3">
      {item.status !== "SUPERSEDED" ? <button onClick={() => onEdit(item)} className="text-xs font-semibold text-[var(--primary-strong)]">Edit</button> : null}
      ...
      {item.status !== "SUPERSEDED" ? <button onClick={() => { const retainsAudit = Boolean(item.fieldApprovedAt || item.previewApprovedAt) || ["FIELD_APPROVED","PREVIEW_PENDING","PUBLISHED","APPROVED"].includes(item.status); const confirmed = window.confirm(retainsAudit ? "Delete this answer from bot use? Its approval history will be retained for audit." : "Delete this draft answer permanently?"); if (confirmed) onReview(item.id, retainsAudit ? "RETIRE" : "DELETE"); }} className="text-xs text-[var(--text-muted)]">Delete</button> : null}
    </div> : null}
  </article>;
}
