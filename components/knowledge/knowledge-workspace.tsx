...
type TenantProfileDraft = { status: "DRAFT" | "NEEDS_REVIEW"; businessName: string; about: string; services: string[]; prices: string[]; hours: string[]; phones: string[]; emails: string[]; addresses: string[]; policies: string[]; team: string[]; socialLinks: string[]; missing: string[]; conflicts: Array<{ key: string; values: string[] }> };
  ...
  return <div className="product-surface min-h-screen">
    ...
    <main className="mx-auto max-w-[1500px] space-y-5 px-5 py-6 sm:px-8">
      ...
      {summary.tenantProfileDraft ? <section className="rounded-2xl border border-[var(--border)] bg-white p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Fact label="Business" value={summary.tenantProfileDraft.businessName} /><Fact label="Public contact" value={[...summary.tenantProfileDraft.phones, ...summary.tenantProfileDraft.emails].join(" · ")} /><Fact label="Services found" value={summary.tenantProfileDraft.services.length ? `${summary.tenantProfileDraft.services.length} structured suggestion${summary.tenantProfileDraft.services.length === 1 ? "" : "s"}` : "Not found"} /><Fact label="Pricing found" value={summary.tenantProfileDraft.prices.length ? `${summary.tenantProfileDraft.prices.length} structured suggestion${summary.tenantProfileDraft.prices.length === 1 ? "" : "s"}` : "Not found"} /></div>
