...
type Summary = { settings: KnowledgeSettings; pages: KnowledgePageSummary[]; propertyId: string | null; documents: KnowledgeDocumentSummary[]; entries: KnowledgeEntrySummary[]; gaps: KnowledgeGapSummary[]; previews?: KnowledgePreviewSummary[]; flags?: KnowledgeFlagSummary[]; verification?: VerificationSummary | null; kbGateEnabled?: boolean };
...
export function KnowledgeWorkspace({
  ...
  async function refreshGovernance() {
    ...
    setSummary((current) => ({ ...current, propertyId: payload.propertyId, documents: payload.documents || [], entries: payload.entries || [], gaps: payload.gaps || [], previews: payload.previews || [], flags: payload.flags || [], verification: payload.verification, kbGateEnabled: payload.kbGateEnabled }));
  }
...
function Status({ value }: { value: string }) { const tone = ["APPROVED","PUBLISHED","RESOLVED"].includes(value) ? "status-success" : ["CONFLICT","REJECTED","INVALID","PAUSED","EXPIRED","OPEN"].includes(value) ? "status-error" : "status-warning"; return <span className={`status-pill ${tone}`}>{value.toLowerCase().replaceAll("_", " ")}</span>; }
