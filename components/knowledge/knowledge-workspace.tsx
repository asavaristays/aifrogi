...
export function KnowledgeWorkspace({
  ...
  return <div className="product-surface min-h-screen">
    ...
    <main className="mx-auto max-w-[1500px] space-y-5 px-5 py-6 sm:px-8">
      {canManage && !summary.documents.length && !summary.entries.length ? <section className="overflow-hidden rounded-lg border border-[#d8c788] bg-[#fffaf0] shadow-[var(--shadow-card)]">
        <div className="border-b border-[#e7dcc0] px-5 py-4"><p className="product-eyebrow">Start here</p><h2 className="mt-1 text-xl font-semibold">Prepare approved knowledge in four steps</h2><p className="mt-1 text-sm text-[var(--text-muted)]">Nothing becomes a live bot answer until you review and approve it.</p></div>
        <ol className="grid gap-px bg-[#e7dcc0] sm:grid-cols-2 xl:grid-cols-4">
          {[["01","Add website","Enter the public business website and save the knowledge controls."],["02","Upload files","Add current PDF, DOCX, TXT, CSV, Markdown or JSON sources below."],["03","Approve facts","Confirm each extracted atomic claim and its customer-facing preview."],["04","Test answers","Ask real customer questions; go live only after the preparation gate is ready."]].map(([number,title,copy]) => <li key={number} className="bg-[#fffdf8] p-5"><span className="text-xs font-black text-[#8a6a16]">{number}</span><strong className="mt-2 block text-sm">{title}</strong><p className="mt-2 text-xs leading-5 text-[var(--text-muted)]">{copy}</p></li>)}
        </ol>
      </section> : null}
