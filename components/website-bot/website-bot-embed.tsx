...
export function WebsiteBotEmbed({ slug, demo = false, botName = "AI Business Assistant", welcomeMessage = "Hello. How can I help with your business enquiry today?", themeColor = "#8a6a16", widgetTheme = "dark", logoUrl = "", welcomeCardImageUrl = "", welcomeCardTitle = "", welcomeCardText = "", showcaseItems = [], dismissible = false, menu = { enabled: false, heading: "", items: [] }, stayAccessToken = "", residentMode = false }: { slug: string; demo?: boolean; botName?: string; welcomeMessage?: string; themeColor?: string; widgetTheme?: "dark" | "light" | "system"; logoUrl?: string; welcomeCardImageUrl?: string; welcomeCardTitle?: string; welcomeCardText?: string; showcaseItems?: ShowcaseItem[]; dismissible?: boolean; menu?: WidgetMenuConfig; stayAccessToken?: string; residentMode?: boolean }) {
  ...
  async function send(event: FormEvent) {
    ...
    const response = await fetch(`/api/public/website-bot/${encodeURIComponent(slug)}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message, sessionId, visitorToken: visitorToken || undefined, stayAccessToken: stayAccessToken || undefined, visitorTimeZone, ...(contactReady ? { name: contactName.trim(), contact: contactValue.trim(), consent: true, requestHuman: true } : {}) }) });
