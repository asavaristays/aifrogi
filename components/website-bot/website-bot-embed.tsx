...
type Message = { role: "visitor" | "bot"; text: string };
...
export function WebsiteBotEmbed({ slug }: { slug: string }) {
  ...
  async function send(event: FormEvent) {
    ...
    const payload = await response.json().catch(() => null) as { answer?: string; error?: string; visitorToken?: string } | null;
    if (payload?.visitorToken) setVisitorToken(payload.visitorToken);
    setMessages((current) => [...current, { role: "bot", text: response.ok ? payload?.answer || "I could not prepare an answer." : payload?.error || "The bot is temporarily unavailable." }]);
    setWaiting(false);
  }
  ...
  return <main className="flex h-screen min-h-[520px] flex-col overflow-hidden rounded-[22px] border border-[#8a6a16]/45 bg-[#101010] text-white">
    ...
    <section aria-live="polite" className="flex-1 space-y-3 overflow-y-auto px-4 py-5">{messages.map((message, index) => <p key={`${message.role}-${index}`} className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "visitor" ? "ml-auto bg-[#8a6a16]" : "bg-white/8 text-white/86"}`}>{message.text}</p>)}{waiting ? <p className="w-fit rounded-2xl bg-white/8 px-4 py-3 text-sm text-[#e2c66d]">AI responding ···</p> : null}</section>
