...
export function WebtechnosysNavigation({ onChat, onFlow, onBooking, menu }: { onChat: () => void; onFlow: (message: string) => void; onBooking: (item: WidgetMenuItem) => void; menu: WidgetMenuConfig }) {
  ...
  if (info) return <nav className={styles.panel} aria-label="In-stay information"><div className={styles.crumbs}><button type="button" onClick={() => setInfo(null)}><ArrowLeft aria-hidden="true" />Back</button><button type="button" onClick={() => { setInfo(null); setSubmenu(null); }}>Main menu</button></div><div className={styles.screen}><h2 ref={heading} tabIndex={-1} className={styles.title}>{info.label}</h2><p className="whitespace-pre-line rounded-2xl border border-[var(--widget-line)] bg-[var(--widget-soft)] p-4 text-sm leading-6 text-[var(--widget-ink)]">{info.value}</p></div><button type="button" className={styles.chat} onClick={onChat}><MessageCircle aria-hidden="true" />Contact front desk</button></nav>;
  return <nav className={styles.panel} aria-label="Bot main menu">
    ...
    <button type="button" className={styles.chat} onClick={onChat}><MessageCircle aria-hidden="true" />Contact front desk</button>{menu.items.some(item => item.action === "LINK") ? <p className={styles.note}>Website pages open in a new tab. Your chat stays here.</p> : null}
  </nav>;
}
