...
const BOT_COLOURS = [
  { label: "Forest", value: "#58785f" },
  { label: "Royal blue", value: "#315f8f" },
  { label: "Terracotta", value: "#a45f45" },
  { label: "Plum", value: "#73546f" },
  { label: "Charcoal", value: "#343b3f" },
  { label: "Gold", value: "#8a6a16" }
] as const;
  ...
  return <section id="bot-appearance" className="rounded-2xl border border-[var(--border)] bg-white p-5 sm:p-6">
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <fieldset><legend className="field-label">Bot colour</legend><div className="mt-2 flex min-h-11 items-center gap-3 rounded-md border border-[var(--border)] px-3"><input aria-label="Custom bot colour" type="color" value={themeColor} disabled={!canManage} onChange={(event) => setThemeColor(event.target.value)} className="h-7 w-9 cursor-pointer border-0 bg-transparent" /><span className="text-xs font-semibold uppercase">{themeColor}</span></div><div className="mt-3 flex flex-wrap gap-2" aria-label="Suggested bot colours">{BOT_COLOURS.map((colour) => <button key={colour.value} type="button" disabled={!canManage} aria-label={`Use ${colour.label}`} aria-pressed={themeColor.toLowerCase() === colour.value} onClick={() => setThemeColor(colour.value)} className="size-7 rounded-md border-2 shadow-sm transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50" style={{ backgroundColor: colour.value, borderColor: themeColor.toLowerCase() === colour.value ? "#111827" : "#ffffff" }} />)}</div><small className="mt-2 block text-xs text-[var(--text-muted)]">Choose a hotel palette or use the colour picker. Readable text is selected automatically.</small></fieldset>
