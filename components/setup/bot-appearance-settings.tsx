...
type Props = {
  initialSettings: Pick<KnowledgeSettings, "welcomeMessage" | "themeColor" | "widgetTheme" | "logoUrl">;
...
export function BotAppearanceSettings({ initialSettings, initialBotName, canManage }: Props) {
  ...
  const canSave = canManage && botName.trim().length > 0 && welcomeMessage.trim().length > 0 && logoIsValid;
  ...
  async function save() {
    ...
    try {
      ...
      const appearanceResponse = await fetch("/api/knowledge", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ welcomeMessage, themeColor, widgetTheme, logoUrl })
      });
