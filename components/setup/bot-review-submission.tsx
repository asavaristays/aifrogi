...
export function BotReviewSubmission({ status, ready, tested, certified, canManage, evidence = emptyEvidence, intakeOnly = false }: { status: string; ready: boolean; tested: boolean; certified: boolean; canManage: boolean; evidence?: KnowledgeEvidence; intakeOnly?: boolean }) {
  ...
  return <section className={`rounded-2xl border p-5 sm:p-6 ${pending ? "border-[#d7c27d] bg-[#fff9e8]" : live ? "border-emerald-200 bg-emerald-50" : "border-[var(--border)] bg-white"}`}>
    ...
    <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--text-muted)]">{live ? "Super Admin approved the bot and your full workspace is available." : pending ? "No further action is required from you. AiFrogi Super Admin will review the information and website, then approve or request one correction." : intakeOnly ? "Submit your onboarding now. Website checking and Excel intelligence can be completed now or later; Super Admin will review for fake or misleading information before approval." : "This gate checks source coverage, reviewed facts, answer testing and the tenant question bank. Do not present the bot to a client until it says Demo ready."}</p>
