...
export function BotReviewSubmission({ status, ready, tested, certified, canManage, evidence = emptyEvidence, intakeOnly = false }: { status: string; ready: boolean; tested: boolean; certified: boolean; canManage: boolean; evidence?: KnowledgeEvidence; intakeOnly?: boolean }) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");
  const pending = status === "REVIEW_PENDING";
  const live = status === "LIVE";
  const blockers = intakeOnly ? [
    !ready ? "Upload and confirm the completed onboarding workbook" : null
  ...
  const demoReady = intakeOnly ? ready && blockers.length === 0 : ready && tested && certified && blockers.length === 0;
  ...
  return <section className={`rounded-2xl border p-5 sm:p-6 ${pending ? "border-[#d7c27d] bg-[#fff9e8]" : live ? "border-emerald-200 bg-emerald-50" : "border-[var(--border)] bg-white"}`}>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="product-eyebrow">{intakeOnly ? "Final owner step" : "Client demonstration gate"}</p><h2 className="mt-1 text-xl font-semibold">{live ? "Your bot is live" : pending ? "Submitted · awaiting AiFrogi review" : intakeOnly && demoReady ? "Hotel information ready" : demoReady ? "Demo ready · submit for review" : intakeOnly ? "Confirm your hotel information" : "Not ready for a client demonstration"}</h2></div><span className={`status-pill ${demoReady || live ? "status-success" : pending ? "status-warning" : "status-error"}`}>{live ? "Live" : pending ? "Under review" : intakeOnly && demoReady ? "Ready to submit" : demoReady ? "Demo ready" : `${blockers.length} action${blockers.length === 1 ? "" : "s"} remaining`}</span></div>
    <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--text-muted)]">{live ? "Super Admin approved the bot and your full workspace is available." : pending ? "No further action is required from you. AiFrogi Super Admin will complete testing, certification and activation." : intakeOnly ? "Submit the confirmed hotel information. AiFrogi Super Admin will complete the technical checks; full workspace tools unlock after approval." : "This gate checks source coverage, reviewed facts, answer testing and the tenant question bank. Do not present the bot to a client until it says Demo ready."}</p>
    {!intakeOnly ? <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
      {[['Website pages (optional)', evidence.pageCount, true], ['Published answers', evidence.published, evidence.published > 0], ['Topic coverage', `${evidence.coveragePercent}%`, ready], ['Knowledge freshness', `${evidence.freshnessRate}%`, evidence.freshnessRate >= 95], ['Conflicting facts', evidence.conflicts, evidence.conflicts === 0], ['Pending approvals', evidence.unsigned + evidence.previewPending, evidence.unsigned + evidence.previewPending === 0], ['Customer test', tested ? 'Completed' : 'Required', tested], ['Question bank', certified ? 'Passed' : 'Required', certified]].map(([label, value, ok]) => <div key={String(label)} className="rounded-xl border border-[var(--border)] bg-white/70 p-3"><p className="text-xs text-[var(--text-muted)]">{label}</p><p className={`mt-1 text-sm font-bold ${ok ? "text-[#126452]" : "text-[#a23b32]"}`}>{String(value)}</p></div>)}
    </div> : null}
    {!pending && !live && canManage ? <button type="button" disabled={saving || !demoReady} onClick={submit} className="mt-4 min-h-11 rounded-full bg-[#8a6a16] px-5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-45">{saving ? "Submitting…" : intakeOnly ? "Submit to AiFrogi Super Admin" : "Approve and submit for review"}</button> : null}
