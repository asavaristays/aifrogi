...
const actions: Array<{ value: WidgetMenuAction; label: string }> = [
  { value: "INFO", label: "In-bot information" }, { value: "LINK", label: "Website link" }, { value: "BOOKING", label: "Booking form" }, { value: "CHAT", label: "Start chat" },
  { value: "CALL", label: "Phone call" }, { value: "EMAIL", label: "Email" }, { value: "SUBMENU", label: "Submenu" }
];
const needsValue = (action: WidgetMenuAction) => ["INFO", "LINK", "BOOKING", "CALL", "EMAIL"].includes(action);
const placeholder = (action: WidgetMenuAction) => action === "INFO" ? "Write a short guest-ready answer shown inside the bot" : ["LINK", "BOOKING"].includes(action) ? "https://example.com/booking" : action === "CALL" ? "+919876543210" : "help@example.com";
  ...
  return <section id="bot-menu" className="scroll-mt-6 rounded-2xl border border-[var(--border)] bg-white p-5 sm:p-6">
    ...
    {menu.enabled ? <div className="mt-5 space-y-4">
      ...
      {menu.items.map((item, index) => <article key={item.id} className="rounded-xl border border-[var(--border)] bg-[#fbfaf7] p-4">
        ...
        {needsValue(item.action) ? <label className="mt-3 block"><span className="field-label">{item.action === "INFO" ? "Information shown inside bot" : "Destination"}</span>{item.action === "INFO" ? <textarea className="product-input mt-2 min-h-24" maxLength={500} value={item.value || ""} disabled={!canManage} placeholder={placeholder(item.action)} onChange={event => update(index, { value: event.target.value })} /> : <input className="product-input mt-2" value={item.value || ""} disabled={!canManage} placeholder={placeholder(item.action)} onChange={event => update(index, { value: event.target.value })} />}</label> : null}
