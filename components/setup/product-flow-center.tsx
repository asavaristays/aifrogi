...
const ownerStyles: Record<ProductFlowOwner, string> = {
  ...
  AiFrogi: "bg-[#fceafb] text-[#97218e]",
  Meta: "bg-[#fff4df] text-[#8a5318]"
};

const statusStyles = {
  complete: "bg-[#e8f7f1] text-[#126452]",
  current: "bg-[#fceafb] text-[#97218e]",
  waiting: "bg-[#f0edf4] text-[#5d5268]",
  blocked: "bg-[#fff0f0] text-[#a3322c]"
};

export function ProductFlowCenter({ flow, mode = "client" }: { flow: ProductFlow; mode?: "client" | "admin" }) {
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-lg bg-[#2c243b] text-white shadow-[var(--shadow-soft)]">
      <div className="grid gap-8 px-6 py-7 lg:grid-cols-[1fr_280px] lg:items-center lg:px-8">
        <div>
          <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-[#ffb4f7]">{flow.phase}</span><span className="text-xs text-white/48">{flow.completedCount} of {flow.totalCount} stages proven</span></div>
          ...
          {flow.nextAction && mode === "client" ? <Link href={flow.nextAction.href} className="mt-6 inline-flex min-h-11 items-center rounded-md bg-[#d92bcb] px-4 text-sm font-semibold text-white hover:bg-[#bb20af]">{flow.nextAction.action}<span aria-hidden="true" className="ml-2">→</span></Link> : null}
        </div>
        <div>
          ...
          <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#e33bd4]" style={{ width: `${flow.progressPercent}%` }} /></div>
...
function StageRow({ stage, index, mode }: { stage: ProductFlowStage; index: number; mode: "client" | "admin" }) {
  ...
  return <li>{mode === "client" && stage.status !== "complete" ? <Link href={stage.href} className="flex min-h-20 items-center gap-4 px-6 py-4 hover:bg-[#fffaff]">{content}</Link> : <div className="flex min-h-20 items-center gap-4 px-6 py-4">{content}</div>}</li>;
}
