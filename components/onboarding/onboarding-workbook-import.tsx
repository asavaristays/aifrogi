...
export function OnboardingWorkbookImport({ organizationId, onImported, hotelTemplate = false }: { organizationId?: string; onImported?: () => void; hotelTemplate?: boolean }) {
  ...
  async function submit(action: "PREVIEW" | "APPLY") {
    ...
    if (action === "PREVIEW") { setPreview(data); setStatus("Preview ready. Check the business details and approved answers before importing."); }
      ...
      let websiteStatus = "Website review will be completed by AiFrogi Super Admin.";
      if (!organizationId) {
        const crawlResponse = await fetch("/api/knowledge", { method: "POST" });
        const crawl = await crawlResponse.json().catch(() => null);
        websiteStatus = crawlResponse.ok
          ? `${crawl.pagesSynced || 0} public website page${crawl.pagesSynced === 1 ? "" : "s"} checked.`
          : "The website needs Super Admin review; your submission is not blocked.";
      }
      setStatus(`${data.stagedCount || 0} answers confirmed. ${websiteStatus}`);
      setPreview(null); setFile(null); if (input.current) input.current.value = ""; onImported?.();
    }
  }
  ...
  return <section className="rounded-lg border border-black/8 bg-white p-6 shadow-sm sm:p-7">
    ...
    <p className="mt-2 text-sm leading-6 text-[#68645c]">Download the template, complete it offline, then preview and confirm it here. AiFrogi checks the public website automatically on this step. Nothing becomes live before Super Admin approval.</p>
