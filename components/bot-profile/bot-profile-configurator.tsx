...
type StoredProfile = { category?: string; operatingMode?: string; channels?: string[]; capabilities?: string[]; humanHandoffEnabled?: boolean; actionApprovalNeeded?: boolean; stayAccessEnabled?: boolean; personaName?: string | null; businessObjective?: string | null; tone?: string | null; languages?: string[]; prohibitedClaims?: string[]; escalationTriggers?: string[]; responseSlaMinutes?: number; reminderPercent?: number; fallbackEnabled?: boolean; safeFallbackMessage?: string | null; status?: string };
...
function normalized(initial?: StoredProfile | null): BotProfileInput {
  ...
  return {
    ...
    stayAccessEnabled: initial?.stayAccessEnabled ?? (category === "STAY"),
    category,
...
export function BotProfileConfigurator({ initialProfile, organizationId, compact = false, websiteOnly = true, onSaved }: { initialProfile?: StoredProfile | null; organizationId?: string; compact?: boolean; websiteOnly?: boolean; onSaved?: (organization: unknown) => void }) {
  ...
  const personaPack = getBotPersonaPack(profile.category);

  function setCategory(category: BotProfileInput["category"]) {
    const pack = getBotPersonaPack(category);
    const categoryBlueprint = getBotBlueprint(category);
    setProfile((current) => ({ ...current, category, stayAccessEnabled: category === "STAY", operatingMode: pack.defaultOperatingMode, capabilities: [...pack.defaultCapabilities], personaName: pack.defaultPersonaName, businessObjective: categoryBlueprint.promise, tone: pack.tone, prohibitedClaims: [...categoryBlueprint.safetyRules], escalationTriggers: [...pack.hardEscalations] }));
  }
  ...
  return <section className={`rounded-lg border border-black/6 bg-white shadow-sm ${compact ? "p-5" : "p-6 sm:p-8"}`}>
    ...
    </div> : <div className="mt-6 rounded-lg border border-[#ded8cb] bg-[#fbfaf7] p-4"><p className="text-sm font-black">Website AI Bot</p><p className="mt-1 text-xs leading-5 text-[#68645c]">Website widget, embed and standalone-link delivery are managed here.</p></div>}
    ...
    {profile.category === "STAY" ? <div className="mt-5 rounded-lg border border-[#d7c27d] bg-[#fff9e8] p-4"><Check label="Enable QR-based HotelGPT in-stay access" checked={profile.stayAccessEnabled} disabled={!superAdminMode} onChange={() => setProfile((current) => ({ ...current, stayAccessEnabled: !current.stayAccessEnabled }))} /><p className="mt-2 text-xs leading-5 text-[#68645c]">This module uses front-desk approval and checkout expiry. It does not require a PMS connector.</p></div> : null}
    ...
    <div className="mt-7 rounded-lg border border-[#ded8cb] bg-[#f8f0d8] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="product-eyebrow">{blueprint.productName} persona pack v{personaPack.version}</p><h3 className="mt-2 text-lg font-black">{blueprint.promise}</h3><p className="mt-2 max-w-3xl text-xs leading-5 text-[#68645c]">{personaPack.identity} Example: “{personaPack.exampleLine}”</p></div><span className="status-pill status-success">versioned</span></div><div className="mt-5 grid gap-5 lg:grid-cols-2"><BlueprintList title="Required conversation slots" items={personaPack.requiredSlots} /><BlueprintList title="Authority map" items={personaPack.authorities.map((item)=>`${item.level}: ${item.capability}`)} /><BlueprintList title="Customer journey" items={personaPack.journey.map((item)=>`${item.step} — ${item.outcome}`)} /><BlueprintList title="Hard escalation boundaries" items={personaPack.hardEscalations} /><BlueprintList title="Information required" items={blueprint.requiredInputs} /><BlueprintList title="Internal business knowledge" items={blueprint.internalKnowledge} /><BlueprintList title="Approved external knowledge" items={blueprint.externalKnowledge} /><BlueprintList title="Connector contracts" items={personaPack.connectors.map((item)=>`${item.name}: reads ${item.reads.join(", ")}; writes ${item.writes.join(", ")}; if unavailable, ${item.unavailableBehavior}`)} /><BlueprintList title="Approved actions" items={blueprint.approvedActions} /><BlueprintList title="Negotiation authority" items={blueprint.negotiationRules} /><BlueprintList title="Safety rules" items={blueprint.safetyRules} /><BlueprintList title="Verified outcomes" items={blueprint.verifiedOutcomes} /><BlueprintList title="Go-live evaluations" items={blueprint.evaluations} /></div></div>
