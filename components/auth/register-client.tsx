...
export function RegisterClient() {
  ...
  return (
    <main className="min-h-screen bg-[#f8f5f9] text-[#2c243b]">
      <header className="border-b border-[#eadfed] bg-white px-5 sm:px-8">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between">
          <Link href="https://aifrogi.com"><Image src="/brand/aifrogi-logo.png" alt="AiFrogi" width={800} height={300} priority className="h-auto w-[150px]" /></Link>
          <Link href="/login" className="text-sm font-semibold text-[#a21c98]"><span className="hidden sm:inline">Already have an account? </span>Sign in</Link>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-10 sm:px-8 lg:grid-cols-[0.75fr_1.25fr] lg:py-14">
        <section className="order-2 lg:order-1 lg:pt-7">
          ...
          <div className="mt-9 divide-y divide-[#e5dce8] border-y border-[#e5dce8]">
        ...
        <section className="order-1 rounded-lg border border-[#e5dce8] bg-white p-6 shadow-[0_18px_55px_rgba(44,36,59,0.08)] sm:p-8 lg:order-2">
          {completeEmail ? (
            <div className="flex min-h-[520px] flex-col justify-center">
              ...
              <div className="mt-7 rounded-md border border-[#eadfed] bg-[#fbf8fc] p-4 text-sm leading-6 text-[var(--text-muted)]">Open the link, create your personal password, then sign in to continue onboarding. Check spam if it does not arrive within a few minutes.</div>
            ...
            <>
              <p className="text-sm font-semibold text-[#c725ba]">Business registration</p>
...
function FlowRow({ number, title, copy }: { number: string; title: string; copy: string }) {
  return <div className="grid grid-cols-[32px_1fr] gap-4 py-5"><span className="flex h-8 w-8 items-center justify-center rounded-md bg-[#fde9fb] text-xs font-semibold text-[#a21c98]">{number}</span><div><strong className="text-sm">{title}</strong><p className="mt-1 text-sm leading-6 text-[var(--text-muted)]">{copy}</p></div></div>;
}
