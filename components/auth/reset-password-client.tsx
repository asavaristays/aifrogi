...
export function ResetPasswordClient({ token }: { token?: string }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f7f4f8] px-5 py-10 text-[#2c243b]">
      <section className="w-full max-w-lg rounded-lg border border-[#eadfed] bg-white p-7 shadow-[0_18px_55px_rgba(44,36,59,0.08)] sm:p-9">
        <Image src="/brand/aifrogi-logo.png" alt="AiFrogi" width={800} height={300} priority className="h-auto w-[170px]" />
        {loading ? (
          <p className="mt-10 text-sm text-[#70697d]">Checking your secure reset link...</p>
        ) : completeEmail ? (
          <div className="mt-10">
            ...
            <p className="mt-3 text-sm leading-6 text-[#70697d]">Your password for <strong>{completeEmail}</strong> has been reset.</p>
            <Link href="/login" className="mt-7 inline-flex min-h-11 w-full items-center justify-center rounded-md bg-[#d92bcb] text-sm font-semibold text-white">Continue to sign in</Link>
          </div>
        ) : info ? (
          <div className="mt-9">
            <p className="text-sm font-semibold text-[#c725ba]">{info.organizationName || "AiFrogi account"}</p>
            <h1 className="mt-2 text-3xl font-semibold">Create a new password</h1>
            <p className="mt-3 text-sm leading-6 text-[#70697d]">Reset password for <strong>{info.email}</strong>. {info.expiresAt ? `This link expires at ${new Date(info.expiresAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}.` : ""}</p>
          ...
          <div className="mt-10">
            ...
            <p className="mt-3 text-sm text-[#70697d]">{error}</p>
            <Link href="/forgot-password" className="mt-6 inline-flex text-sm font-semibold text-[#a21c98]">Request a new reset link</Link>
          </div>
        )}
      </section>
    </main>
