...
const inbox = readFileSync(resolve(process.cwd(), "app/(app)/team-inbox/page.tsx"), "utf8");
...
test("team inbox is restricted to Website Bot conversations", () => {
  assert.match(inbox, /lead\.websiteSession/);
  assert.doesNotMatch(inbox, /loadWhatsAppIntegration/);
});
