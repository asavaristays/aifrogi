...
const categories = ["EDUCATION", "REAL_ESTATE", "FLOWCART", "CUSTOM"];
...
const outagePrompts: Record<string, string> = {
  EDUCATION: "Book data programme counselling tomorrow at 11 AM; the calendar is offline.",
  REAL_ESTATE: "Arrange a site visit to the Porvorim apartment Sunday at 3 PM; the calendar is offline.",
  FLOWCART: "Order one chocolate cake for delivery; the store is offline.",
  CUSTOM: "Create a maintenance request for an urgent leak in office room 2; the system is offline."
};
for (const scenario of BUCKET_THREE_CASES.filter(c => categories.includes(c.category) && !journeys.includes(c.journey))) {
  test(`${scenario.id} ${scenario.journey}`, async () => {
    profile.category = scenario.category;
