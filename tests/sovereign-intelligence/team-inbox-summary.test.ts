...
test.beforeEach(()=>{state.allowed=false;state.origin=true;state.queries=[];state.writes=[];state.identity=null;});
...
test('summary queries are tenant and operator scoped',async()=>{state.allowed=true;const r=await GET();assert.equal(r.headers.get('cache-control'),'private, no-store');const b=await r.json();assert.equal(b.unread,3);assert.equal(b.needsHuman,1);assert.deepEqual(state.identity,{kind:'tenant',organizationId:'org',actor:'team-inbox-summary:owner@test.invalid'});assert.equal(state.queries[0].where.organizationId,'org');assert.equal(state.queries[0].where.actorEmail,'owner@test.invalid');assert.equal(state.queries[1].where.lead.propertyId,'owned');assert.equal(state.queries[2].where.propertyId,'owned');});
