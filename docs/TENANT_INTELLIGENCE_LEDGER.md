...
| `TI-011` | Per-bot Flow Intelligence templates and Main Menu publishing | `LOCAL` | Per tenant bot | `/flow-intelligence`, tenant settings, website widget Main Menu | Initial template configurator is live in `tenant-flow-intelligence-20260909`; n8n-style node canvas, user-defined nodes/connections and enabled sidebar icon are implemented locally and await deployment. |
