...
| `TI-011` | Per-bot Flow Intelligence templates and Main Menu publishing | `PILOT_EVIDENCE` | Per tenant bot | `/flow-intelligence`, tenant settings, website widget Main Menu | Release `tenant-flow-intelligence-20260909` is live. Four governed templates implement answer-first → human chat → consented callback; client flow creation and visitor use require pilot evidence. |
