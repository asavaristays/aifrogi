...
| `TI-011` | Per-bot Flow Intelligence templates and Main Menu publishing | `PILOT_EVIDENCE` | Per tenant bot | `/flow-intelligence`, tenant settings, website widget Main Menu | Release `ti011-visual-builder-20260909` is live with an enabled icon, bounded n8n-style node canvas, user-defined nodes/connections, validation and Main Menu publication. Real client flow use remains to be reviewed. |
