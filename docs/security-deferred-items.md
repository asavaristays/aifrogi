...
| Client-bundle secret check | Secrets must never ship to the browser bundle. This is a cheap regression check and catches accidental `NEXT_PUBLIC_*` misuse. | Before GA and whenever new secrets/integrations are added. | Automated grep/build artifact check confirms Meta secrets, tokens, SMTP passwords, Twilio secrets, session secrets, and webhook secrets are absent from `.next/static` and client chunks. |
...
| Session/device controls | OTP protects privileged sign-in, but customers eventually need visibility into active sessions and forced logout. | Before multi-user teams become common. | Users can view active sessions/devices, revoke sessions, and admins can force logout compromised accounts. |
