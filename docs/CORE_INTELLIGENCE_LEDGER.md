...
| `CI-013` | Conclusive missing-answer recovery | `PILOT_EVIDENCE` | Every website bot | public website-bot runtime and widget | Release `ci013-conclusive-fallback-20260909`: unverified output is withheld; the bot supplies the configured business phone when present, requests consented callback details, records the lead and routes to Team Inbox when handover is enabled. Awaiting real visitor-to-inbox pilot confirmation. |
| `CI-014` | Sustained-interest callback capture | `LOCAL` | Every lead-capable website bot | `lib/lead-qualification.ts`, website widget | The first interest turn is answered normally; two relevant commercial-interest turns offer one optional, consented private callback. Direct quote/booking requests remain immediate and unrelated questions cannot trigger capture. Awaiting VPS deployment and Webtechnosys pilot evidence. |
