...
| `CI-013` | Conclusive missing-answer recovery | `LOCAL` | Every website bot | public website-bot runtime and widget | Unverified output is not shown; the bot supplies the configured business phone when present, requests consented callback details, records the lead and routes to Team Inbox when handover is enabled. Awaiting deployment. |
