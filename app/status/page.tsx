...
export default function StatusPage() {
  return (
    <main className="bg-white text-[#2c243b]">
      <SiteHeader />
      <section className="bg-[#2c243b] px-5 py-16 text-white sm:px-8 sm:py-24"><div className="mx-auto max-w-7xl"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#ff8af1]">Service status</p><h1 className="mt-4 max-w-4xl text-4xl font-semibold tracking-[-.04em] sm:text-6xl">Current health. Clear ownership.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-white/62">Check whether AiFrogi is reachable and see where to go when messaging or workflows need attention.</p></div></section>
      ...
      <section className="border-y border-black/8 bg-[#fbf8fc] px-5 py-20 sm:px-8"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.7fr_1.3fr]"><div><p className="product-eyebrow">Support commitment</p><h2 className="mt-3 text-3xl font-semibold tracking-[-.03em]">The urgent issue reaches the right queue.</h2></div><div className="border-y border-black/10">{[
      ...
      <section className="px-5 py-16 sm:px-8"><div className="mx-auto flex max-w-7xl flex-col gap-5 bg-[#2c243b] px-6 py-8 text-white sm:flex-row sm:items-center sm:justify-between sm:px-8"><div><h2 className="text-2xl font-semibold">Need help now?</h2><p className="mt-2 text-sm text-white/55">Include the workspace, expected result, actual result, and approximate time—never a password, OTP, or token.</p></div><a href="mailto:info@aifrogi.com?subject=AiFrogi%20support" className="inline-flex min-h-12 shrink-0 items-center justify-center bg-[#d92bcb] px-5 text-sm font-bold">Email support</a></div></section>
      <SiteFooter />
    </main>
  );
}
