...
export async function POST(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  const evidence = await db.sovereignAnswerEvidence.findFirst({ where: { id: payload.evidenceId, propertyId: session.propertyId, leadId: token.leadId } });
  if (!evidence) return NextResponse.json({ error: "This answer does not belong to the active conversation." }, { status: 404, headers });
  const signal = helpful ? null : normalizeImprovementSignal({ propertyId: session.propertyId, type: "NEGATIVE_FEEDBACK", text: payload.reason });
  const storedReason = signal ? encodeImprovementSignalReason(signal, payload.reason) : payload.reason?.trim().slice(0, 1000) || null;
  const feedback = await db.$transaction(async (transaction) => {
    const saved = await transaction.sovereignAnswerFeedback.upsert({
      where: { evidenceId: evidence.id },
      create: { propertyId: session.propertyId, evidenceId: evidence.id, leadId: token.leadId, helpful, reason: storedReason },
      update: { helpful, reason: storedReason }
    });
    if (!helpful) {
      const reviewClassification = evidence.failureClassification === "NONE" || evidence.failureClassification === "SAFE_ESCALATION" ? "UNCLASSIFIED_NEGATIVE_FEEDBACK" : evidence.failureClassification;
      await transaction.sovereignAnswerEvidence.update({ where: { id: evidence.id }, data: { safeResolution: false, failureClassification: reviewClassification } });
      await transaction.sovereignReplayCase.upsert({
        where: { sourceEvidenceId: evidence.id },
        create: {
          propertyId: session.propertyId,
          sourceEvidenceId: evidence.id,
          personaCategory: evidence.personaCategory,
          personaVersion: evidence.personaVersion,
          anonymizedQuestion: anonymizeReplayText(evidence.question),
          anonymizedPriorTurns: [],
          expectedDisposition: evidence.disposition === "ANSWER" ? "REVIEW_REQUIRED" : evidence.disposition,
          expectedClaimIds: evidence.usedClaimIds.length ? evidence.usedClaimIds : evidence.retrievedClaimIds,
          failureClassification: reviewClassification
        },
        update: {
          personaCategory: evidence.personaCategory,
          personaVersion: evidence.personaVersion,
