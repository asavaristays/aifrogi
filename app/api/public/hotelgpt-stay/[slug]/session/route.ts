...
export async function POST(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  const body = await request.json().catch(() => null) as { mode?: unknown; roomNumber?: unknown; phoneLast4?: unknown; guestName?: unknown; phoneNumber?: unknown; checkIn?: unknown; checkOut?: unknown } | null;
  const response = await withPublicBotDatabaseContext(slug, async () => {
    ...
    if (body?.mode === "LOGIN") {
      ...
      if (!roomNumber || phoneLast4.length !== 4) return NextResponse.json({ error: "Enter the approved room number and last 4 digits of the registered phone." }, { status: 400, headers });
      ...
      const item = rows[0];
      if (!item || item.phoneNumber.replace(/\D/g, "").slice(-4) !== phoneLast4) return NextResponse.json({ error: "No active approved stay matches these details. Ask the front desk to verify your registration." }, { status: 404, headers });
