...
export async function POST(request:Request){
  ...
  const slug=await getCurrentWorkspaceSlug();const settings=await readKnowledgeSettings(slug);let replies=(settings.hotelQuickReplies||[]).filter(item=>item.journey==="PRE_STAY");
  ...
  if(payload?.action==="save"){
    const normalized=normalizeHotelQuickReply(payload.reply);if(!normalized||normalized.journey!=="PRE_STAY")return NextResponse.json({error:"Only Pre-Stay saved replies are supported. In-Stay replies are written by the front desk."},{status:400});
