    ...
    try {
      ...
      const mail = await sendBookingMail({
        ...
        attachments: [
          ...
          { filename: "aifrogi-logo.png", content: await (await fetch(`${appUrl}/brand/aifrogi-logo-transparent.png`)).arrayBuffer().then((value) => Buffer.from(value)), cid: "aifrogi-logo", contentType: "image/png" }
        ]
      });
