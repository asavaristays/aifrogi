...
async function context() {
  ...
  if (!access.ok || !canPerformGovernedKnowledgeAction(access.role, "CREATE_CLAIM")) return null;
  return {access, property: access.property};
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    else if (payload?.action === "EDIT_AND_APPROVE") result = await editAndApproveKnowledgeClaim({ ...base, entryId: payload.id || "", question: payload.question || "", answer: payload.answer || "", category: payload.category || "General" });
