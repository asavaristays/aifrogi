...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [payment, order, plans] = await Promise.all([fetchVerifiedRazorpayPayment(paymentId), fetchRazorpayOrder(orderId), withClientDatabaseContext(access, "billing-checkout-verify", ensureBillingPlans)]);
    ...
    const { invoice, notification } = await withClientDatabaseContext(access, "billing-checkout-verify", async () => {
