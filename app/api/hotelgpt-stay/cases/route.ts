...
export async function GET(request: Request) {
  ...
  return withTenantDatabaseContext({ kind: "tenant", organizationId: access.organization.id, actor: `in-stay-cases:${access.user.username}` }, async () => {
    ...
    const leads = await db.lead.findMany({ where: { propertyId: access.propertyId, stayLabel: { startsWith: "In-stay · Room " } }, include: { messages: { orderBy: { sentAt: "asc" }, take: 50 } }, orderBy: { lastActivityAt: "desc" }, take: 200 });
    const cases = leads.map(lead => ({ id: lead.id, guestName: lead.name, room: lead.stayLabel.replace("In-stay · Room ", ""), phone: lead.phone, kind: lead.intent === "IN_STAY_COMPLAINT" ? "COMPLAINT" : "QUERY", status: lead.stage === "BOOKED" ? "RESOLVED" : lead.stage === "CONTACTED" ? "IN_PROGRESS" : "NEW", priority: lead.isHighPriority ? "URGENT" : "NORMAL", lastActivityAt: lead.lastActivityAt.toISOString(), latestMessage: lead.messages.at(-1)?.body || "" }));
