...
import { grantFreeAiCredits } from "@/lib/ai-credits";
...
export async function PATCH(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  ...
  try {
    if (action === "GRANT_FREE_CREDITS") {
      const expiresAt = payload?.expiresAt ? new Date(String(payload.expiresAt)) : null;
      if (expiresAt && Number.isNaN(expiresAt.getTime())) throw new Error("Enter a valid credit expiry date.");
      const grant = await grantFreeAiCredits({ organizationId, credits: Number(payload?.credits), reason: String(payload?.reason || ""), expiresAt, actorEmail: user.username });
      const notification = await notifyBillingEvent({ organizationId, targetId: grant.id, actorEmail: user.username, event: "FREE_CREDITS_GRANTED", description: `${grant.credits.toLocaleString("en-IN")} free AI reply credits`, value: "₹0 · Super Admin grant", reference: grant.reason, validity: grant.expiresAt });
      return NextResponse.json({ grant, allocatedImmediately: true, notification });
    }
    if (action !== "UPDATE_POLICY") return NextResponse.json({ error: "Unsupported action" }, { status: 400 });
