...
export default function DashboardError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return <main className="flex min-h-screen items-center justify-center bg-[var(--background)] px-5 py-12">
    <section className="w-full max-w-xl rounded-lg border border-[var(--border)] bg-white p-7 shadow-[var(--shadow-card)]">
      ...
      <p className="mt-3 text-sm leading-6 text-[var(--text-muted)]">Your WhatsApp connection and message delivery continue independently. Retry the dashboard first; if the problem remains, open a support request and AiFrogi will attach your account context.</p>
