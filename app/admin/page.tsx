...
export default async function AdminPage() {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-8">
    ...
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><AdminMetric label="Customers live" value={String(live)} helper={`${waiting} waiting`} /><AdminMetric label="Open support" value={String(openTickets.length)} helper={`${urgentTickets.length} high priority`} /><AdminMetric label="Messages" value={String(messageCount)} helper={`${failedMessages} failures`} /><AdminMetric label="Campaign runs" value={String(campaignCount)} helper="Persisted and auditable" /><AdminMetric label="Tech Provider" value="Verified" helper="Embedded onboarding eligible" /></section>
