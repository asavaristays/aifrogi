...
import { getSupportTicket, markSupportTicketViewed } from "@/lib/repositories/support-repository";
...
export default async function AdminSupportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const [{ id }, user] = await Promise.all([params, getCurrentPlatformAdmin()]);
  if (!user) notFound();
  const ticket = await withPlatformAdminDatabaseContext(user, "admin-support-detail", () => getSupportTicket(id)); if (!ticket) notFound();
  const sla = supportSlaState(ticket);
  await withPlatformAdminDatabaseContext(user, "admin-support-detail", async () => {
    await logSupportDataAccess({ organizationId: ticket.organizationId, actorEmail: user.username, scope: "CONVERSATIONS", targetType: "SUPPORT_TICKET", targetId: ticket.id, granted: true, summary: "Support viewed content explicitly submitted in this ticket; other client data remains access-controlled." });
    await markSupportTicketViewed({ ticketId: ticket.id, viewer: "ADMIN" });
  });
